<template>
  <view class="he-page-content" :data-theme="theme">
    <view class="he-sticky">
      ...
      <view class="he-switch flex">
        ...
        <view class="flex-sub" id="he-switch--2" :class="{ active: currentIndex === 2 }" @click="clickTab(2)"
          >视频</view
        >
        <view class="he-line" :style="[tabBarStyle]"></view>
      </view>
    </view>
    <view class="he-list">
      <view class="he-item" v-for="(item, index) in list" :key="index">
        <view class="he-line-4">
          {{ item.name }}
        </view>
        ...
        <view class="he-video" v-else-if="item.type === 2"> </view>
    ...
    <he-no-content-yet v-if="isNothing" text="暂未相关素材"></he-no-content-yet>
...
export default {
  ...
  methods: {
    init() {
      this.getTabRect();
      promotermaterial().then(response => {
    ...
    clickTab(index) {
      ...
      this.scrollByIndex();
      promotermaterial(1, index).then(response => {
