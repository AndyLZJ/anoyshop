<template>
    <view class="he-page-content" :data-theme="theme">
        <view class="he-item" v-for="(item, index) in list" :key="index" @click="navigateTo('/pages/order/after-sales-details?id=' + item.id)">
            <view class="he-item__top flex justify-between align-center">
                <view class="he-status__sign">
                    <view class="iconfont he-sign" :class="
              item.type === 0
            ...
            <view class="he-item__body flex">
                ...
                <view class="he-item__content">
                    <view class="he-item__name he-line-1">{{
                        item.goods.goods_name
                        }}</view>
                    <view class="he-item__attr">{{ item.goods.show_goods_param }}</view>
                    <view class="he-item__number-price" v-if="item.order_type=='task'">
                        <text class="he-item__number">x{{ item.return_number }}</text>
                        <text class="he-item__price" v-if="item.type !== 2">退款：
                            <template v-if="item.return_score_type">
                                {{item.return_score}} 积分 +
                            </template>
                            ¥{{ item.return_amount }}
                        </text>
                    </view>
                    <view class="he-item__number-price" v-else>
                        <text class="he-item__number">x{{ item.return_number }}</text>
          ...
          item.status === 121 ||
          item.status === 131
        " class="he-item__footer flex align-center" :class="
          item.type !== 2 && item.status === 200
            ? 'justify-between'
            : 'justify-end'
        ">
                ...
                <view>
                    <button class="cu-btn he-btn-over" v-if="item.status >= 200" @click.stop="setClose(item)">
                        删除记录
                    </button>
                    <button class="cu-btn he-btn-fill" @click.stop="
              navigateTo(
...
export default {
    ...
    onLoad() {
        ...
        this.getList().then(function(res) {
            ...
            _this.loadStatus =
                _this.list.length < _this.page.size ? "nomore" : "loadmore";
        });
    },
    onReachBottom() {
        let _this = this;
