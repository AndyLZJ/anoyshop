  ...
  <view class="detail-product-info">
    ...
    <view class="he-goods" :data-theme="theme">
      <view class="he-item" v-for="(item, index) in goods" :key="index">
        <view
          class="he-body flex"
          @tap="navigateToDetail(`/pages/goods/detail?id=${item.goods_id}`)"
        >
          ...
          <view class="he-item__content flex">
            <view class="he-item-content-content">
              ...
              <view class="he-item__attr he-line-2">{{
            ...
            <view class="flex-sub he-item-content-price">
              <template
                v-if="Number(item.total_amount) > Number(item.pay_amount)"
              >
                <view class="flex align-center justify-end">
                  <text class="he-item-price-prompt">优惠后</text>
                  <text class="he-item-payAmount">￥{{ Math.floor(item.pay_amount/item.goods_number*100)/100 }}</text>
                </view>
        ...
        <view
          class="he-footer flex justify-end"
          v-if="status === 201 || status === 202 || status === 203"
        >
          <button
...
export default {
  ...
  methods: {
    ...
    navigateTo: function (id) {
      uni.navigateTo({
        url:
          "/pages/order/after-sales-details?id=" + id + "&behavior=order_goods",
      });
    },
  },
  filters: {
