    ...
    <view class="ad" v-if="display" :class="{'ad_element':current>0}">
        <view class="ad-item-photo" :style="{ width: '100%', height: '100vh' }" v-if="contentData.length">
          ...
          right: facade.indicator_align == 'right',
                ...
                <view class="ad-pointer_item" v-for="(item, index) in contentData" :key="index" v-if="facade.indicator_style != 3">
                    <button :class="{ is_active: index === current }" :style="{
              backgroundColor:
                index === current ? facade.indicator_color : 'none',
            }"></button>
                </view>
...
export default {
    ...
    methods: {
        ...
        getCover(cover = "") {
            ...
            if (cover) {
                if (
                    Object.prototype.toString.call(cover) === "[object Array]" &&
                    cover[0]
                ) {
                    return cover[0].url;
                }
