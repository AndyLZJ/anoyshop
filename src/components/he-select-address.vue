    ...
    <he-popup mode="bottom" v-model="showModal" :borderRadius="16">
        <view class="he-select-address" :data-theme="theme">
            ...
            <view class="he-body flex align-center">
                ...
                <view class="he-body__item" v-if="provinceIndex !== -1" @click="active = 1" :class="active === 1 ? 'he-body__item-1' : 'he-body__item-0'">{{city}}</view>
                <view class="he-body__item" v-if="cityIndex !== -1" @click="active = 2" :class="active === 2 ? 'he-body__item-1' : 'he-body__item-0'">{{district}}</view>
            </view>
            <view class="he-footer">
                <template v-if="list.length > 0">
                    <scroll-view class="he-scroll-view" scroll-y>
                        <view class="he-scroll-item" v-if="active === 0" @click="() => {setActive('provinceIndex', index)}" :class="provinceIndex === index ? 'he-scroll-item-1' : 'he-scroll-item-0'" v-for="(item, index) in list" :key="index">{{item.name}}</view>
                        <template v-if="provinceIndex !== -1">
                            <view class="he-scroll-item" v-if="active === 1" @click="() => {setActive('cityIndex', index)}" :class="cityIndex === index ? 'he-scroll-item-1' : 'he-scroll-item-0'" v-for="(item, index) in list[provinceIndex].list" :key="index">{{item.name}}</view>
                            <template v-if="cityIndex !== -1">
                                <view class="he-scroll-item" v-if="active === 2" @click="() => {setActive('districtIndex', index)}" :class="districtIndex === index ? 'he-scroll-item-1' : 'he-scroll-item-0'" v-for="(item, index) in list[provinceIndex].list[cityIndex].list" :key="index">{{item.name}}</view>
                            </template>
                        </template>
                    </scroll-view>
                </template>
